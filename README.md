...
**Data.** 3,264 T1-weighted MRI images from Bhuvaji et al. (2020), split into 2,870 training and 394 test images across four classes.
...
**Evaluation.** Models were compared to a frequentist multinomial logit baseline (`nnet::multinom`) on the held-out test set. Classification metrics include accuracy, per-class precision/recall/F1, and macro-averaged F1. Bayesian-specific outputs include posterior credible intervals and predictive entropy.
...
**Key finding.** With a weakly-informative prior (Normal(0, 10)) and this much training data, the posterior mode coincides with the maximum-likelihood estimate, so Bayesian Model A reproduces the frequentist point predictions almost exactly — the expected behavior when the likelihood dominates a diffuse prior. The moderately-regularizing prior (Normal(0, 2)) departs slightly, yielding a small gain in macro and glioma F1. The practical value of the Bayesian framework in this setting is therefore not improved accuracy but uncertainty quantification: credible intervals for coefficients and predictive entropy for individual test cases.
...
├── 02_preprocess.R             # Grayscale, resize, Otsu skull-strip, z-score
...
2. Download the Bhuvaji Brain Tumor Classification (MRI) dataset from Kaggle and extract it into `data/raw/` (it contains `Training/` and `Testing/` folders, with one subfolder per class). The raw images are not redistributed here; `data/raw/` is git-ignored.
3. Install the required R packages: `EBImage` (Bioconductor), `coda`, `nnet`, `mvtnorm`, `ggplot2`, `dplyr`.
4. Run the scripts in numerical order, `01_catalog_images.R` through `05_generate_figures.R`.

## Limitations

- Feature extraction used whole-brain radiomics rather than lesion-specific regions of interest.
- Leave-one-out cross-validation was not performed due to compute constraints; a held-out test set was used instead.
- Classical radiomic features substantially underperform deep-learning benchmarks (~90%) on this task; this project deliberately focuses on Bayesian methodology and uncertainty quantification rather than state-of-the-art classification accuracy.
